...
function runtimeState(runtime, readinessSnapshot) {
	if (readinessSnapshot && typeof readinessSnapshot === "object") {
		...
		if (!(identity?.execution_plane === "hub-3210" && identity.profile === "dsh-crew" && Number(identity.listen_port) === 3210 && typeof identity.runtime_id === "string" && identity.runtime_id.trim())) return READINESS_STATES.UNAVAILABLE;
		if (typeof runtime?.runtime_id === "string" && runtime.runtime_id.trim() && runtime.runtime_id !== identity.runtime_id) return READINESS_STATES.UNKNOWN;
	...
	return runtime?.ok === true && runtime.service === "dsh-crew-hub" && runtime.execution_plane === "hub-3210" && runtime.profile === "dsh-crew" && Number(runtime.listen_port) === 3210 && typeof runtime.runtime_id === "string" && runtime.runtime_id.trim() ? READINESS_STATES.READY : READINESS_STATES.UNKNOWN;
}
...
function modelCallabilityState(projection, runtime = null, now = Date.now()) {
	...
	if (!(projection.schema_version === 1 && Number.isFinite(projection.captured_at) && projection.captured_at <= now && typeof projection.current_runtime_id === "string" && projection.current_runtime_id.trim() && (!runtime || projection.current_runtime_id === runtime.runtime_id))) return READINESS_STATES.UNKNOWN;
	if (projection.overall === "CALLABLE") {
		...
		return Number.isFinite(projection.expires_at) && projection.expires_at > now && typeof projection.current_runtime_id === "string" && projection.current_runtime_id.trim() && (!runtime || projection.current_runtime_id === runtime.runtime_id) && activeRoles.length > 0 && activeRoles.every((role) => projection.roles?.[role]?.state === "CALLABLE") ? READINESS_STATES.READY : READINESS_STATES.UNKNOWN;
	}
...
function WorkersPanel({ ctx }) {
	...
	const [readinessSnapshot, setReadinessSnapshot] = (0, react.useState)(void 0);
	...
	* (DSH's setting may be unset), and the hub localizes its own strings from it. */
	...
	const refreshAll = (0, react.useCallback)(async () => {
		try {
			...
			if (ext?.ok) setReadinessSnapshot(ext.extension?.readiness_snapshot ?? void 0);
			else setReadinessSnapshot(void 0);
		} catch {}
	}, [get]);
	(0, react.useEffect)(() => {
