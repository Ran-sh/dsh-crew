...
function modelCallabilityState(projection, runtime = null, now = Date.now()) {
	...
	if (!(projection.schema_version === MODEL_CALLABILITY_SCHEMA_VERSION && Number.isFinite(projection.captured_at) && projection.captured_at <= now && typeof projection.current_runtime_id === "string" && projection.current_runtime_id.trim() && isCompleteRuntimeIdentity(projection.runtime_identity) && isCompleteRuntimeIdentity(runtime) && sameCompleteRuntimeIdentity(projection.runtime_identity, runtime) && projection.current_runtime_id === projection.runtime_identity.runtime_id)) return READINESS_STATES.UNKNOWN;
	if (projection.overall === "CALLABLE") {
		const activeRoles = projection.enabled_roles && typeof projection.enabled_roles === "object" ? Object.entries(projection.enabled_roles).filter(([, enabled]) => enabled === true).map(([role]) => role) : [];
		const allowedRoles = /* @__PURE__ */ new Set(["worker", "reviewer"]);
		return Number.isFinite(projection.expires_at) && projection.expires_at > now && typeof projection.current_runtime_id === "string" && projection.current_runtime_id.trim() && isCompleteRuntimeIdentity(projection.runtime_identity) && isCompleteRuntimeIdentity(runtime) && sameCompleteRuntimeIdentity(projection.runtime_identity, runtime) && projection.current_runtime_id === projection.runtime_identity.runtime_id && activeRoles.length > 0 && Object.keys(projection.enabled_roles ?? {}).every((role) => allowedRoles.has(role)) && Object.keys(projection.roles ?? {}).every((role) => allowedRoles.has(role)) && activeRoles.every((role) => projection.roles?.[role]?.state === "CALLABLE") && Object.entries(projection.enabled_roles ?? {}).every(([role, enabled]) => enabled === true || projection.roles?.[role]?.state === "NOT_APPLICABLE") ? READINESS_STATES.READY : READINESS_STATES.UNKNOWN;
	}
	if (projection.overall === "NOT_CALLABLE") {
		...
		const allowed = /* @__PURE__ */ new Set(["worker", "reviewer"]);
		return active.length > 0 && Object.keys(enabled).every((role) => allowed.has(role)) && Object.keys(roles).every((role) => allowed.has(role)) && Object.entries(enabled).every(([role, value]) => value === true || roles[role]?.state === "NOT_APPLICABLE") && active.some((role) => roles[role]?.state === "NOT_CALLABLE" && Number.isFinite(roles[role]?.expires_at) && roles[role].expires_at > now) ? READINESS_STATES.UNAVAILABLE : READINESS_STATES.UNKNOWN;
	}
...
function acceptReadinessResponse(response, { generation, latestGeneration } = {}) {
	...
	return {
		accepted: true,
		envelope: response?.ok === true && isExactNativeCrewIdentity(extensionRuntime) && isExactNativeCrewIdentity(snapshot?.runtime) && sameCompleteRuntimeIdentity(extensionRuntime, snapshot.runtime) ? {
