window.__ModuleLoader__.load({ id: "dsh-crew", factory: (require) => {
...
const COPY = {
	zh: {
		label: "DSH Crew",
		title: "DSH Crew",
		intro: "让 Claude Code / Codex 把子任务派给 DeepSeek V4 Flash / Pro worker：在宿主里以原生子代理形态出现，进度可见，会话则跑在本 DSH 实例内。下面配置派发默认值、执行方式与多模态桥，并可一键装进宿主。",
		integrations: "集成",
		installed: "已安装",
		notInstalled: "未安装",
		hud: "HUD 段",
		...
		cardMM: {
			t: "视觉与生图",
			d: "让纯文本的 DeepSeek 借订阅的眼睛和画笔：describe_image、会话贴图转写与 generate_image 都用这里的设置。"
		},
		cardCustomProv: {
			t: "自定义 Provider",
	...
	en: {
		label: "DSH Crew",
		title: "DSH Crew",
		intro: "Let Claude Code / Codex delegate subtasks to DeepSeek V4 Flash / Pro workers: they appear as native subagents in the host with live progress, while their sessions run inside this DSH instance. Configure dispatch defaults, execution and the multimodal bridge below, and install into the host in one click.",
		...
		cardMM: {
			...
			d: "Lends the text-only DeepSeek model eyes and a brush: describe_image, pasted-image transcription and generate_image all use these settings."
		},
		cardCustomProv: {
			t: "Custom providers",
...
function WorkersPanel({ ctx }) {
	...
	*/
	const readJson = (0, react.useCallback)(async (res, path) => {
		...
		if (text === "") throw new Error(res.status === 404 || res.status === 405 ? `${copy.staleHub}（${path} → HTTP ${res.status}）` : `${path} → HTTP ${res.status}，响应为空`);
		try {
			return JSON.parse(text);
		} catch {
			throw new Error(`${path} → 非 JSON 响应: ${text.slice(0, 160)}`);
		}
	}, [copy]);
	const get = (0, react.useCallback)(async (path) => readJson(await fetch(`${API}${path}`, { cache: "no-store" }), path), [readJson]);
	const post = (0, react.useCallback)(async (path, body) => readJson(await fetch(`${API}${path}`, {
		method: "POST",
	...
	const refreshAll = (0, react.useCallback)(async () => {
		try {
			...
			if (pr.ok) setPresetOptions([{
				value: "default",
				label: `default · 跟随 DSH${pr.defaultId ? ` (${pr.defaultId})` : ""}`
			}, ...(pr.presets ?? []).map((x) => ({
				value: x.id,
				label: x.name ?? x.id
			}))]);
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("tbody", { children: jobs.map((job) => {
							...
							return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("tr", { children: [
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("td", {
									...
									title: `耗时 ${elapsed(job.startedAt, job.endedAt)} · 第${job.turn}轮第${job.step}步 · ${job.toolCalls}次工具调用`,
									children: [
