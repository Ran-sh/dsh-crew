...
function acceptReadinessResponse(response, { generation, latestGeneration } = {}) {
	...
	const projection = valid ? validateModelCallabilityV2({
		...
		expectedSelections: {
			worker: snapshot.worker?.selected,
			reviewer: snapshot.reviewer?.selected
		}
	}) : { ok: false };
	return {
		accepted: true,
	...
	* (DSH's setting may be unset), and the hub localizes its own strings from it. */
	...
	(0, react.useEffect)(() => {
		...
		const timer = setInterval(() => {
			const generation = ++readinessGeneration.current;
			Promise.all([
				get("/jobs", 5e3).catch(() => null),
				get("/provider-health", 5e3).catch(() => null),
				get("/extension", 5e3).catch(() => null)
			]).then(([j, health, ext]) => {
