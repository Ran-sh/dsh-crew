...
function isCompleteRuntimeIdentity(identity) {
	return Boolean(identity && typeof identity === "object" && nonEmpty(identity.execution_plane) && nonEmpty(identity.profile) && Number.isInteger(identity.listen_port) && identity.listen_port > 0 && nonEmpty(identity.runtime_id));
}
function sameCompleteRuntimeIdentity(left, right) {
	return isCompleteRuntimeIdentity(left) && isCompleteRuntimeIdentity(right) && left.execution_plane === right.execution_plane && left.profile === right.profile && left.listen_port === right.listen_port && left.runtime_id === right.runtime_id;
}
function isExactNativeCrewIdentity(identity) {
	return isCompleteRuntimeIdentity(identity) && identity.execution_plane === "hub-3210" && identity.profile === "dsh-crew" && identity.listen_port === 3210;
}
//#endregion
...
const MAX_RUNTIME_SNAPSHOT_AGE_MS = 3e5;
...
function runtimeState(runtime, readinessSnapshot) {
	if (readinessSnapshot && typeof readinessSnapshot === "object") {
		...
		if (!isExactNativeCrewIdentity(identity)) return READINESS_STATES.UNAVAILABLE;
		if (runtime && (!isCompleteRuntimeIdentity(runtime) || !sameCompleteRuntimeIdentity(runtime, identity))) return READINESS_STATES.UNKNOWN;
		if (!Number.isFinite(readinessSnapshot.captured_at)) return READINESS_STATES.UNKNOWN;
		if (readinessSnapshot.captured_at > Date.now() + 3e4) return READINESS_STATES.UNKNOWN;
		if (Date.now() - readinessSnapshot.captured_at > MAX_RUNTIME_SNAPSHOT_AGE_MS) return READINESS_STATES.DEGRADED;
		const row = Array.isArray(readinessSnapshot.readiness_matrix?.rows) ? readinessSnapshot.readiness_matrix.rows.find((entry) => entry?.id === "hub_compatibility") : void 0;
	...
	return runtime?.ok === true && isExactNativeCrewIdentity(runtime) ? READINESS_STATES.READY : READINESS_STATES.UNKNOWN;
}
...
function projectHostReadiness({ installStatus, runtime, surface, readinessSnapshot } = {}) {
	...
	return [
		...
		{
			id: "crew_harness",
			state: runtimeState(runtime, readinessSnapshot),
...
const SOURCES = /* @__PURE__ */ new Set(["provider_health", "execution"]);
const MAX_EVIDENCE_TTL_MS = 864e5;
...
function route(value) {
	return typeof value?.provider === "string" && value.provider.trim() && typeof value?.model === "string" && value.model.trim() ? {
		provider: value.provider.trim(),
		model: value.model.trim()
	} : null;
}
function validEvidence(role, now) {
	if (!SOURCES.has(role.source) || !Number.isFinite(role.observed_at) || role.observed_at > now || !Number.isFinite(role.expires_at) || role.expires_at <= now || role.expires_at - role.observed_at > MAX_EVIDENCE_TTL_MS) return false;
	if (role.source === "execution") {
		if (!role.last_success || typeof role.last_success !== "object" || typeof role.last_success.job_id !== "string" || !role.last_success.job_id.trim() || role.last_success.observed_at !== role.observed_at || role.last_success.expires_at !== role.expires_at) return false;
	}
	return true;
}
function validateModelCallabilityV2({ projection, runtime, expectedEnabledRoles, expectedSelections, now = Date.now() } = {}) {
	const fail = (reason_code) => ({
		ok: false,
		reason_code,
		state: "UNKNOWN"
	});
	if (!projection || typeof projection !== "object" || projection.schema_version !== 2) return fail("MODEL_CALLABILITY_SCHEMA_INVALID");
	if (!isExactNativeCrewIdentity(runtime) || !isExactNativeCrewIdentity(projection.runtime_identity) || !sameCompleteRuntimeIdentity(runtime, projection.runtime_identity) || projection.current_runtime_id !== runtime.runtime_id) return fail("MODEL_CALLABILITY_RUNTIME_MISMATCH");
	...
	for (const roleName of ROLES) {
		...
		if (role.selected !== void 0 && role.selected !== null && !selected) return fail("MODEL_CALLABILITY_SELECTION_INVALID");
		if (expectedSelections) {
			...
			if (expected === null !== (selected === null) || expected && (expected.provider !== selected.provider || expected.model !== selected.model)) return fail("MODEL_CALLABILITY_SELECTION_MISMATCH");
		}
	...
	const state = activeStates.includes("NOT_CALLABLE") ? "NOT_CALLABLE" : activeStates.length > 0 && activeStates.every((value) => value === "CALLABLE") ? "CALLABLE" : activeStates.includes("STALE") ? "STALE" : "UNKNOWN";
	...
	return {
		ok: true,
		state,
		reason_code: "MODEL_CALLABILITY_VALID"
	};
}
//#endregion
//#region src/client/model-callability-view.mjs
function modelCallabilityState(projection, runtime = null, now = Date.now()) {
	if (!projection || typeof projection !== "object" || typeof projection.overall !== "string") return READINESS_STATES.UNKNOWN;
...
//#endregion
//#region src/client/readiness-envelope.mjs
function readinessExpiryDelay(expiresAt, now = Date.now()) {
...
*/
function acceptReadinessResponse(response, { generation, latestGeneration } = {}) {
	...
	const valid = response?.ok === true && isExactNativeCrewIdentity(extensionRuntime) && isExactNativeCrewIdentity(snapshot?.runtime) && sameCompleteRuntimeIdentity(extensionRuntime, snapshot.runtime);
	...
	return {
		accepted: true,
		envelope: valid && projection.ok ? {
			runtime: extensionRuntime,
			snapshot,
			expiresAt: snapshot.model_callability.expires_at
		} : {}
	};
}
//#endregion
...
function aggregateModelInvocations(jobs = []) {
	...
	for (const job of recentJobs) {
		...
		if (job?.role === "worker" || job?.role === "reviewer") current.roles.add(job.role);
...
//#endregion
//#region src/client/panel-chrome.tsx
...
function PanelHeader({ title, eyebrow, description, href, linkText, children }) {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("header", {
		className: "crew-panel-header",
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
			className: "crew-panel-heading",
			children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: "crew-eyebrow",
					children: eyebrow
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h2", {
					className: "crew-panel-title",
					children: title
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
					className: "crew-panel-description",
					children: description
				}),
				children && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: "crew-panel-badges",
					children
				})
			]
...
const API$1 = "/_dsh/dsh-crew/history/";
const pending = (phase) => !!phase && ![
	"IDLE",
	"DONE",
	"FAILED",
	"ROLLED_BACK"
...
const copy = {
	zh: {
		title: "工作区与会话清理",
		intro: "仅影响 3210 的工作区记录与会话日志；不删除项目文件、附件或 3080 数据。",
		operation: "操作",
		archive: "归档（可恢复）",
		delete: "删除（不可恢复）",
		scope: "范围",
		...
		before: "指定时间之前",
		time: "创建时间早于",
		timeHint: "按本机时区选择，严格按创建时间筛选；包含新会话的工作区会保留。",
		preview: "预览清理范围",
		confirm: "确认执行",
		acknowledgement: "删除确认：请输入 DELETE",
		idle: "会短暂停止并重启 3210；请先结束或关闭所有正在使用的 3210 会话。",
		consent: "我已确认范围，并同意短暂重启 3210",
		restore: "恢复",
		archives: "已归档批次",
		empty: "暂无可恢复的归档",
		working: "维护中，正在等待 3210 重新连接…",
		refresh: "刷新页面与工作区列表",
		recover: "恢复未完成的维护",
		recovery: "维护尚未完成，请使用恢复按钮；若 3210 无法连接，在终端运行 dsh-crew history recover。",
		cancelQueued: "取消尚未开始的维护",
		cancelQueuedHint: "仅取消尚未开始的维护，不清理任何数据；如果操作已开始，请等待完成。",
		count: (w, s) => `${w} 个工作区 · ${s} 个会话`,
		confirmRestore: "恢复这一批归档？将短暂重启 3210，不会覆盖冲突数据。",
		phase: {
			IDLE: "就绪",
			QUEUED: "已提交",
			STOPPING: "停止后台",
	...
	en: {
		title: "Workspace & session cleanup",
		intro: "Only 3210 workspace records and session logs. Project files, attachments and 3080 data are untouched.",
		operation: "Action",
		archive: "Archive (restorable)",
		delete: "Delete (permanent)",
		scope: "Scope",
		all: "Everything (includes your own sessions)",
		crew: "Crew-created only (recommended)",
		worktree: "Crew worktrees only",
		before: "Before a date",
		time: "Created before",
		timeHint: "Local timezone; strict creation-time cutoff. Workspaces with newer sessions are kept.",
		preview: "Preview cleanup",
		confirm: "Confirm operation",
		acknowledgement: "Type DELETE to confirm deletion",
		idle: "3210 will briefly stop and restart. End or close all active 3210 conversations first.",
		consent: "I reviewed the scope and agree to restart 3210",
		restore: "Restore",
		archives: "Archived batches",
		empty: "No restorable archives",
		working: "Maintenance in progress; waiting for 3210 to reconnect…",
		refresh: "Reload page and workspace list",
		recover: "Recover unfinished maintenance",
		recovery: "Maintenance is unfinished. Use recovery; if 3210 is unreachable, run dsh-crew history recover in a terminal.",
		cancelQueued: "Cancel unstarted maintenance",
		cancelQueuedHint: "Cancel only if maintenance has not started. No data is cleaned; if already running, wait for completion.",
		count: (w, s) => `${w} workspaces · ${s} sessions`,
		confirmRestore: "Restore this archive? 3210 will briefly restart; conflicting data will not be overwritten.",
		phase: {
			IDLE: "Ready",
			QUEUED: "Queued",
			STOPPING: "Stopping backend",
			APPLYING: "Processing data",
...
function HistoryPanel({ locale }) {
	const t = copy[locale === "zh" ? "zh" : "en"];
	const [open, setOpen] = (0, react.useState)(false);
	const [operation, setOperation] = (0, react.useState)("archive");
	const [scope, setScope] = (0, react.useState)("crew");
	...
	const [ack, setAck] = (0, react.useState)("");
	...
	async function request(action, body) {
		const res = await fetch(API$1 + action, {
			cache: "no-store",
			...body === void 0 ? {} : {
				method: "POST",
	...
	async function act(action, body) {
		...
		try {
			...
			setNotice(String(error.message));
			setPreview(null);
		} finally {
			setBusy(false);
		}
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("summary", {
			...
			children: [
				...
				scope === "before" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: [
					t.time,
					" ",
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					type: "button",
					style,
					disabled: locked || !connected || scope === "before" && !Number.isFinite(new Date(before).getTime()),
					onClick: () => void act("preview", {
						operation,
						scope,
						...scope === "before" ? { before: new Date(before).toISOString() } : {}
					}),
					children: t.preview
				}),
				preview && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: t.count(preview.counts.workspaces, preview.counts.sessions) }),
						preview.before && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [
							t.time,
							": ",
							new Date(preview.before).toLocaleString()
						] }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("details", { children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("summary", { children: t.preview }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("ul", { children: preview.items?.map((item) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("li", { children: [
								item.title,
								" ",
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: item.id })
							] }, item.id)) }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								style: {
								...
								children: [preview.sessionIds?.slice(0, 100).join(", "), preview.sessionIds?.length > 100 ? " …" : ""]
							})
						] }),
						!preview.executable && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
							role: "alert",
							children: preview.blockedReason
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", { children: t.idle }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
								type: "checkbox",
								checked: consent,
						...
						operation === "delete" && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", { children: [
							t.acknowledgement,
							" ",
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
								style,
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							style,
							disabled: locked || !consent || !preview.executable || operation === "delete" && ack !== "DELETE",
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					"aria-live": "polite",
					children: [
						t.phase[status.phase] ?? status.phase,
				...
				!connected && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					role: "status",
					children: [pending(status.phase) ? t.working : "3210 history API unavailable", /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: t.recovery })]
				}),
				["RECOVERY_REQUIRED", "QUEUED"].includes(status.phase) && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: status.phase === "QUEUED" ? t.cancelQueuedHint : t.recovery }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					type: "button",
					style,
					disabled: busy,
					onClick: () => {
						if (window.confirm(status.phase === "QUEUED" ? t.cancelQueuedHint : t.recovery)) act("recover", { confirm: true });
					},
					children: status.phase === "QUEUED" ? t.cancelQueued : t.recover
				})] }),
				["DONE", "ROLLED_BACK"].includes(status.phase) && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					type: "button",
					style,
					onClick: () => window.location.reload(),
					children: t.refresh
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: t.archives }),
				archives.length === 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					style: { opacity: .6 },
					children: t.empty
				}),
				archives.map((a) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					style: {
						...style,
						display: "flex",
						gap: 8,
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						style: { flex: 1 },
						children: a.invalid ? `${a.id} · ${a.code}` : `${new Date(a.createdAt).toLocaleString()} · ${t.count(a.workspaces, a.sessions)}`
					}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						type: "button",
						style,
						disabled: locked || a.invalid,
						onClick: () => {
							if (window.confirm(t.confirmRestore)) act("restore", {
								archiveId: a.id,
								confirm: true
							});
						},
...
const SCHEDULE_DEFAULT_OFFSET = 480;
...
function normalizeSchedule(value) {
	...
	const unset = (field) => field === void 0 || field === null;
	const offset = unset(source.timezone_offset_minutes) ? SCHEDULE_DEFAULT_OFFSET : Number.isInteger(source.timezone_offset_minutes) && Math.abs(source.timezone_offset_minutes) <= 840 ? source.timezone_offset_minutes : null;
	const weekdays = unset(source.weekdays) ? [
		1,
		2,
		3,
		4,
		5
	] : Array.isArray(source.weekdays) ? [...new Set(source.weekdays.filter((d) => Number.isInteger(d) && d >= 0 && d <= 6))].sort((a, b) => a - b) : null;
	const windows = unset(source.peak_windows) ? [{
		start: "09:00",
		end: "12:00"
	}, {
		start: "14:00",
		end: "18:00"
	}] : Array.isArray(source.peak_windows) ? source.peak_windows.filter((w) => scheduleClock(w?.start) !== null && scheduleClock(w?.end) !== null && scheduleClock(w.start) !== scheduleClock(w.end)).map((w) => ({
		start: String(w.start).trim(),
		end: String(w.end).trim()
	})) : null;
	const seen = /* @__PURE__ */ new Set();
	const models = (Array.isArray(source.models) ? source.models : []).filter((row) => {
	...
	if (offset === null || weekdays === null || windows === null) return {
		timezone_offset_minutes: SCHEDULE_DEFAULT_OFFSET,
		weekdays: [],
		peak_windows: [],
		models
	};
...
function scheduleIsPeak(schedule, now = /* @__PURE__ */ new Date()) {
	if (schedule.weekdays.length === 0) return false;
	const shifted = new Date(now.getTime() + schedule.timezone_offset_minutes * 6e4);
	...
	for (const window of schedule.peak_windows) {
		...
		if (!(start < end ? minutes >= start && minutes < end : minutes >= start || minutes < end)) continue;
...
const COPY = {
	zh: {
		...
		notInstalled: "未安装",
		ready: "集成就绪",
		integrationReady: "集成就绪",
		modelCallable: "模型可调用",
		modelNotCallable: "模型未验证",
		modelUnknown: "模型未知",
		notReady: "未就绪",
		hud: "HUD 段",
		install: "安装",
		...
		readinessLabels: {
			...
			zcode_mcp: "ZCode MCP",
			crew_harness: "Crew plugin profile",
		...
		harnessProviders: "Harness Providers",
		harnessProviderHint: "DeepSeek 官方 Harness 的 3210 dsh-crew profile 中的真实 Provider 注册与生命周期；不是视觉 / 生图适配器。",
		providerState: (state) => `状态：${state}`,
		providerModels: (count) => `${count} 个模型`,
		providerJobs: (count) => `${count} 个运行任务`,
		providerOfficialBuiltin: "· 官方内置（不可删除）",
		providerSourceUnresolved: "· 来源未解析（安全锁定）",
		providerLayerMigration: "检测到旧版 profile Provider",
		providerLayerMigrationHint: "这些 Provider 仍在 Crew profile/base 层，所以 3210 原生页面不会显示删除。请先按迁移计划提升到 Harness 用户设置层；不会复制密钥。",
		providerLayerMigrationPlan: "迁移计划",
		providerLayerMigrate: "迁移到用户层",
		providerLayerMigrating: "迁移中…",
		providerLayerMigrationConfirm: "确认把这个 Provider 移到 Harness 用户设置层？会先保存可回滚快照，不会复制密钥值。",
		providerLayerRollback: "回滚迁移",
		providerDeletePlan: "生成删除计划",
		providerDelete: "确认删除",
		providerPlanReady: "计划已生成；再次确认后才会修改 3210 配置。",
		providerRollback: "回滚",
		providerRollbackConfirm: "按已保存事务恢复这个 Harness Provider 及原有路由？",
		providerRollbackUnavailable: "没有可验证的回滚事务",
		...
		recoveryUnresolved: "发现不可自动恢复的事务",
		recoveryStorage: "存储项",
		recoveryQuarantine: "隔离",
		recoveryQuarantineConfirm: "该事务清单无法安全恢复。将其移入隔离区并阻止自动使用，继续？",
		...
		credentialPurgeUnverified: "已执行清理，但验证失败；引用仍保留用于恢复检查",
		customProviders: "多模态适配器",
		addProvider: "＋ 添加多模态适配器",
		...
		healthGate: "健康门",
		healthGateDesc: {
			"hard-failures": "跳过明确不可调用",
			off: "关闭健康门"
		},
		reviewGate: "Reviewer 验收门",
		reviewGateDesc: {
			required: "required · 未批准则失败",
			optional: "optional · 记录但不阻断",
			off: "off · 不自动复审"
		},
		isolation: "工作区隔离",
		...
		scheduleTitle: "波峰波谷调度",
		scheduleHint: "按本地时间限制指定模型：波峰时被“阻止”的模型会被跳过，由下一个候选接替任务；“提醒”的仍会使用并在选择轨迹中标记。未列出的模型不受限制。默认时段为 DeepSeek 公布的波峰时间（工作日 09:00-12:00、14:00-18:00）。",
		scheduleTimezone: "时区（相对 UTC）",
		scheduleWindows: "波峰时段",
		schedulePeak: "现在处于波峰",
		scheduleOffPeak: "现在处于波谷",
		scheduleAddWindow: "添加时段",
		scheduleStart: "开始",
		scheduleEnd: "结束",
		scheduleBlock: "阻止",
		scheduleWarn: "提醒",
		scheduleOff: "关闭",
		scheduleRestricted: "受限模型",
		scheduleNoRestricted: "还没有受限模型；未列出的模型不受时间限制。",
		scheduleWeekdays: "生效工作日",
		scheduleBoundary: "下次派发生效",
		modelActivity: "模型活动概览",
		modelActivityHint: "聚合本 Hub 内存中最近 500 个含真实调用证据的任务；每个模型按任务计数（不是推测底层轮次），最多显示 50 个模型。不保存提示词、结果或凭据。",
		noModelActivity: "还没有真实模型活动。",
		calls: "任务数",
	...
	en: {
		...
		modelNotCallable: "model not verified",
		...
		sectionNames: {
			...
			schedule: "Peak / off-peak scheduling",
		...
		harnessProviders: "Harness Providers",
		harnessProviderHint: "Live Provider registration and lifecycle in the official DeepSeek Harness dsh-crew profile on 3210; separate from vision / image-gen adapters.",
		providerState: (state) => `State: ${state}`,
		providerModels: (count) => `${count} models`,
		providerJobs: (count) => `${count} running jobs`,
		providerOfficialBuiltin: "· official built-in (immutable)",
		providerSourceUnresolved: "· source unresolved (locked)",
		providerLayerMigration: "Legacy profile Providers detected",
		providerLayerMigrationHint: "These Providers still exist in the Crew profile/base layer, so the native 3210 page cannot show Delete. Promote them to Harness user settings with an explicit plan; keys are never copied.",
		providerLayerMigrationPlan: "Migration plan",
		providerLayerMigrate: "Migrate to user layer",
		providerLayerMigrating: "Migrating…",
		providerLayerMigrationConfirm: "Move this Provider into Harness user settings? A rollback snapshot is saved first; credential values are never copied.",
		providerLayerRollback: "Rollback migration",
		...
		providerRollback: "Rollback",
		providerRollbackConfirm: "Restore this Harness Provider and its previous routing from the saved transaction?",
		providerRollbackUnavailable: "No verified rollback transaction is available",
		...
		recoveryUnresolved: "An unrecoverable recovery transaction is blocking deletion",
		recoveryStorage: "storage entry",
		recoveryQuarantine: "Quarantine",
		recoveryQuarantineConfirm: "This recovery manifest cannot be safely reopened. Move it into the quarantine area and keep it out of automatic recovery?",
		...
		credentialPurgeUnverified: "Purge ran but verification failed; the reference remains visible for recovery",
		customProviders: "Multimodal adapters",
		addProvider: "＋ Add multimodal adapter",
		...
		modelOrdering: "Candidate ordering",
		orderingDesc: {
			manual: "Manual order",
		...
		healthGateDesc: {
			"hard-failures": "Skip known hard failures",
			off: "Health gate off"
		},
		reviewGate: "Reviewer acceptance gate",
		reviewGateDesc: {
			required: "required · fail unless approved",
			optional: "optional · record without blocking",
			off: "off · no automatic review"
		},
		...
		scheduleTitle: "Peak / off-peak scheduling",
		scheduleHint: "Restricts chosen models by local wall clock. During peak, a \"block\" model is skipped and the next candidate serves the job; a \"warn\" model is still used and flagged in the selection trace. Models not listed are unrestricted. Defaults are DeepSeek's published peak hours (weekdays 09:00-12:00 and 14:00-18:00).",
		scheduleTimezone: "Timezone (relative to UTC)",
		scheduleWindows: "Peak windows",
		schedulePeak: "Peak hours now",
		scheduleOffPeak: "Off-peak now",
		...
		scheduleWarn: "Warn",
		scheduleOff: "Off",
		scheduleRestricted: "Restricted models",
		scheduleNoRestricted: "No restricted models yet; unlisted models have no time limit.",
		scheduleWeekdays: "Days the windows apply",
		scheduleBoundary: "Effective for the next dispatch",
		modelActivity: "Model activity overview",
		modelActivityHint: "Aggregates the latest 500 in-memory Hub jobs with real model-activity evidence; each model is counted once per qualifying job (not an inferred turn count). Shows at most 50 models; prompts, results, and credentials are never stored here.",
		noModelActivity: "No real model activity yet.",
		calls: "Jobs",
...
function CollapsibleSection({ sectionId, title, summary, expanded, onToggle, children }) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
			type: "button",
			className: "crew-section-trigger",
			...
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					...
					className: "crew-section-summary",
					children: summary
				})]
			})]
		}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
...
function readinessChip(state) {
	const color = state === READINESS_STATES.READY ? "#3fb950" : state === READINESS_STATES.DEGRADED ? "#c98735" : state === READINESS_STATES.UNAVAILABLE ? "#f85149" : state === READINESS_STATES.NOT_APPLICABLE ? "#7d8590" : "inherit";
	return {
		fontSize: 10.5,
		fontWeight: 650,
		padding: "1px 7px",
...
function MinimalCrewPanel({ locale, surface, runtime }) {
	...
	const hint = native ? zh ? "这里是 DeepSeek 官方 Harness 的专用 dsh-crew profile，DSH Crew 作为插件加载其中。请使用 Harness 原生菜单管理 Provider、Harness Models、Agent 预设与底层配置。" : "This is the dedicated dsh-crew profile of the official DeepSeek Harness, with DSH Crew loaded as a plugin. Use the native Harness menus for Providers, Harness Models, Agent presets, and low-level settings." : zh ? "无法用结构化后端证据确认当前界面，因此已保守隐藏 Crew 编排、任务与宿主集成功能。" : "Structured backend evidence could not identify this surface, so Crew orchestration, jobs, and host integrations are conservatively hidden.";
			...
			children: [
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						...
						children: native ? "3210 · official profile + Crew plugin" : "surface · unknown"
					})]
				}),
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("a", {
					...
					children: zh ? "打开官方 Harness 界面（3080） →" : "Open official Harness (3080) →"
				})
			]
		})
	});
...
function WorkersPanel({ ctx }) {
	...
	const [credentialUnverified, setCredentialUnverified] = (0, react.useState)([]);
	...
	const [readinessEnvelope, setReadinessEnvelope] = (0, react.useState)({});
	const readinessGeneration = (0, react.useRef)(0);
	...
	* (DSH's setting may be unset), and the hub localizes its own strings from it. */
	...
	const get = (0, react.useCallback)(async (path, timeoutMs = 8e3) => readJson(await fetch(withLang(path), {
		cache: "no-store",
	...
	const refreshAll = (0, react.useCallback)(async () => {
		...
		try {
			...
			if (cr?.ok) {
				setCredentialRefs(cr.records ?? []);
				setCredentialUnverified(cr.unverified_purges ?? []);
			}
		} catch {
	...
	(0, react.useEffect)(() => {
		...
		const poll = async () => {
			const generation = ++readinessGeneration.current;
			try {
				const ext = await get("/extension", 5e3);
				if (!cancelled && generation === readinessGeneration.current) {
					const accepted = acceptReadinessResponse(ext, {
						generation,
						latestGeneration: readinessGeneration.current
					});
			...
			Promise.all([get("/jobs", 5e3).catch(() => null), get("/provider-health", 5e3).catch(() => null)]).then(([j, health]) => {
				if (cancelled) return;
				if (j?.ok) setJobs(j.jobs ?? []);
	...
	const refreshProviderInventory = (0, react.useCallback)(async () => {
		...
		try {
			...
			if (health?.ok) setProviderHealth(health.health ?? []);
			if (refs?.ok) {
				setCredentialRefs(refs.records ?? []);
				setCredentialUnverified(refs.unverified_purges ?? []);
			}
		} catch (error) {
	...
	*  legacy 3080 supervisor endpoint is retired. */
	...
	const rolePolicyField = (role, patch) => {
		const effectivePatch = role === "worker" && (patch.ordering === "manual" || patch.ordering === "health-aware") ? {
			...patch,
			adaptive: {
				...normalizeAdaptive(config?.worker?.model_policy?.adaptive),
				enabled: patch.ordering === "health-aware"
			}
		} : patch;
		setConfig((current) => ({
			...current,
			[role]: {
				...current?.[role],
				model_policy: {
					...current?.[role]?.model_policy,
					...effectivePatch
				}
			}
		}));
		applyPatch({ [role]: { model_policy: effectivePatch } });
	};
	const reviewField = (patch) => {
		setConfig((current) => ({
			...current,
			review: {
				...current?.review,
	...
	const deleteHarnessProvider = async (record) => {
		...
		if (record.references?.harness_default === true) {
			const candidate = (providerInventory?.records ?? []).find((item) => item.id === "deepseek-official" && item.id !== record.id && item.desired_state !== "absent" && (item.models?.length ?? 0) > 0)?.id ?? (providerInventory?.records ?? []).find((item) => item.id !== record.id && item.desired_state !== "absent" && item.delete_capability !== "source-unresolved" && (item.models?.length ?? 0) > 0)?.id ?? "";
	...
	const migrateHarnessProvider = async (entry) => {
		...
		try {
			...
			const result = await post(`/providers/${encoded}/migrate`, {
				plan_id: planned.plan.plan_id,
				confirm: true
			});
			if (!result.ok) throw new Error(result.code ?? result.error ?? copy.providerLifecycleError);
	...
	const rollbackHarnessMigration = async (record, transactionId) => {
		...
		try {
			const encoded = encodeURIComponent(record.id);
			const result = await post(`/providers/${encoded}/rollback-migration`, {
	...
	const rollbackHarnessProvider = async (record, transactionId, transactionState = "VERIFIED") => {
		...
		try {
			...
			} else if (result.state !== "ROLLED_BACK") throw new Error(result.code ?? copy.providerLifecycleError);
			setNotice(copy.saved);
			await refreshProviderInventory();
		} catch (error) {
			try {
	...
	const quarantineRecoveryTransaction = async (entry) => {
		...
		setNotice(copy.working);
		try {
			const result = await post("/providers/_recovery/quarantine", {
				action_id: actionId,
				confirm: true
			});
			if (!result.ok) throw new Error(result.code ?? result.error ?? copy.providerLifecycleError);
	...
	const integrationRow = (name, installed, ready, extra, installTarget, uninstallTarget, tips) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				style: S.chip(ready),
				children: ready ? `● ${copy.integrationReady}` : `○ ${copy.notReady}`
			}),
			extra,
	...
	const saveAdaptive = (candidate) => {
		...
		applyPatch({ worker: { model_policy: {
			adaptive: next,
			ordering
		} } });
	};
	const schedule = normalizeSchedule(config?.model_schedule);
	const saveSchedule = (candidate) => {
		const next = normalizeSchedule(candidate);
		setConfig((current) => ({
			...current,
			model_schedule: next
		}));
		applyPatch({ model_schedule: next });
	};
	const schedulePeakNow = scheduleIsPeak(schedule);
	const scheduleModeOf = (ref) => schedule.models.find((entry) => entry.provider === ref.provider && entry.model === ref.model)?.mode ?? "off";
	const setScheduleMode = (ref, mode) => {
		const others = schedule.models.filter((entry) => !(entry.provider === ref.provider && entry.model === ref.model));
		saveSchedule({
			...schedule,
			models: mode === "off" ? others : [...others, {
				...ref,
				mode
			}]
		});
	...
	const scheduleModelRefs = (() => {
		...
		for (const tier of ["flash", "pro"]) for (const ref of config?.[`${tier}_model_priority`] ?? []) if (ref?.provider && ref?.model) seen.set(`${ref.provider}\0${ref.model}`, ref);
	...
	const toggleScheduleWeekday = (day) => {
		const days = schedule.weekdays.includes(day) ? schedule.weekdays.filter((value) => value !== day) : [...schedule.weekdays, day].sort((a, b) => a - b);
		saveSchedule({
			...schedule,
			weekdays: days
		});
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)(PanelStyles, {}),
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)(PanelHeader, {
				title: copy.title,
				eyebrow: copy.surfaceLabel,
				description: copy.intro,
				href: CREW_CONTROL_PLANE_URL,
				linkText: copy.openOfficial,
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						style: S.chip(status?.codex?.installed === true),
						children: `Codex ${status?.codex?.installed === true ? copy.installed : copy.notInstalled}`
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						style: S.chip(status?.claude?.installed === true),
						children: `Claude ${status?.claude?.installed === true ? copy.installed : copy.notInstalled}`
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						style: S.chip(status?.zcode?.installed === true),
						children: `ZCode ${status?.zcode?.installed === true ? copy.installed : copy.notInstalled}`
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						style: readinessChip(modelState),
						children: modelState === READINESS_STATES.READY ? copy.modelCallable : modelState === READINESS_STATES.UNKNOWN ? copy.modelUnknown : copy.modelNotCallable
					}),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						style: S.chip(jobs.some((job) => job.status === "running")),
						children: copy.runningCount(jobs.filter((job) => job.status === "running").length)
					})
				]
			}),
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)(HistoryPanel, { locale }),
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "crew-config-toolbar",
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						style: {
							...S.section,
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)(CollapsibleSection, {
				...
				summary: sectionSummary(...[
					...
					"Claude",
					"ZCode"
				].map((name) => `${name} ${status?.[name.toLowerCase()]?.installed ? copy.installed : copy.notInstalled}`), modelState === READINESS_STATES.READY ? copy.modelCallable : copy.modelNotCallable),
			...
			config && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)(CollapsibleSection, {
							...
							children: [
								...
								block({
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										style: S.fieldLabel,
										children: copy.reviewGate
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CustomSelect, {
										value: config.review?.gate ?? "required",
										onChange: (v) => reviewField({ gate: v }),
										options: [
											"required",
											"optional",
											"off"
											...
											value,
											label: copy.reviewGateDesc[value]
										}))
									})]
								})] })),
								block({
									t: copy.modelOrdering,
									d: copy.orderingHint
								}, /* @__PURE__ */ (0, react_jsx_runtime.jsx)(react_jsx_runtime.Fragment, { children: ["worker", "review"].map((role) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
										...
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
											style: S.fieldLabel,
											children: [
												role === "worker" ? copy.roleWorker : copy.roleReviewer,
												" · ",
												copy.modelOrdering
											]
										}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CustomSelect, {
											value: config?.[role]?.model_policy?.ordering ?? "manual",
											onChange: (v) => rolePolicyField(role, { ordering: v }),
											options: ["manual", "health-aware"].map((value) => ({
												value,
												label: copy.orderingDesc[value]
											}))
										})]
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
										style: S.field,
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
											...
											value: config?.[role]?.model_policy?.health_gate ?? "hard-failures",
											onChange: (v) => rolePolicyField(role, { health_gate: v }),
											options: ["hard-failures", "off"].map((value) => ({
												value,
												label: copy.healthGateDesc[value]
											}))
										})]
									})]
								}, role)) })),
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)(CollapsibleSection, {
					sectionId: "schedule",
					title: copy.sectionNames.schedule,
					summary: sectionSummary(schedulePeakNow ? copy.schedulePeak : copy.scheduleOffPeak, `UTC${schedule.timezone_offset_minutes >= 0 ? "+" : "-"}${Math.abs(schedule.timezone_offset_minutes) / 60}`, config?.model_schedule === void 0 ? copy.scheduleOff : `${schedule.models.length} · ${copy.scheduleRestricted}`),
					expanded: !!expandedSections.schedule,
					onToggle: () => toggleSection("schedule"),
					children: block({
						t: copy.scheduleTitle,
						d: copy.scheduleHint
					}, /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
							style: S.field,
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								style: S.fieldLabel,
								children: copy.scheduleTimezone
							}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CustomSelect, {
								value: String(schedule.timezone_offset_minutes),
								onChange: (v) => saveSchedule({
									...schedule,
									timezone_offset_minutes: Number(v)
								}),
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								children: copy.scheduleWeekdays
							}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
								style: {
									display: "flex",
									gap: 5,
								...
								children: scheduleWeekdayNames.map((name, day) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									style: {
										...S.btn,
										padding: "3px 9px",
										opacity: schedule.weekdays.includes(day) ? 1 : .45
									},
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								children: [schedule.peak_windows.map((window, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									style: {
										display: "flex",
										gap: 6,
										alignItems: "center"
									...
									children: [
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
											...
											style: S.input,
											onChange: (event) => {
												const next = schedule.peak_windows.map((row, i) => i === index ? {
													...row,
										...
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
											...
											onClick: () => saveSchedule({
												...schedule,
												peak_windows: schedule.peak_windows.filter((_, i) => i !== index)
											}),
											children: "×"
										})
									]
								}, `${window.start}-${window.end}-${index}`)), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									style: {
										...S.btn,
										alignSelf: "flex-start"
									...
									onClick: () => saveSchedule({
										...schedule,
										peak_windows: [...schedule.peak_windows, {
											start: "09:00",
											end: "12:00"
										}]
									}),
									children: copy.scheduleAddWindow
								})]
							})]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									...
									children: copy.scheduleRestricted
								}),
								scheduleModelRefs.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									style: {
										fontSize: 11.5,
										opacity: .6
									},
									children: copy.scheduleNoRestricted
								}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									style: {
										display: "flex",
										flexDirection: "column",
									...
									children: scheduleModelRefs.map((ref) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										style: {
											display: "flex",
											gap: 8,
											alignItems: "center",
										...
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
											...
											options: [
												...
												{
													value: "warn",
													label: copy.scheduleWarn
												},
												{
													value: "block",
													label: copy.scheduleBlock
												}
											]
										})]
									}, `${ref.provider}\0${ref.model}`))
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									style: {
										display: "block",
									...
									children: copy.scheduleBoundary
								})
							]
						})
					] }))
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)(CollapsibleSection, {
					...
					children: [
						...
						!providerInventoryError && (providerInventory?.migration?.providers?.length ?? 0) > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							style: {
								border: "1px solid rgba(201,135,53,0.45)",
								borderRadius: 7,
								padding: "8px 10px",
								marginBottom: 8
							},
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									...
									children: copy.providerLayerMigration
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									style: {
										fontSize: 11.5,
									...
									children: copy.providerLayerMigrationHint
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									style: {
										fontSize: 11,
									...
									children: (providerInventory.migration.providers ?? []).map((entry) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										style: {
											display: "flex",
											alignItems: "center",
											gap: 6,
										...
										children: [
											...
											" · ",
											entry.action,
											entry.harness_default ? " · Harness Default" : "",
											/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { style: { flex: 1 } }),
											["promote-existing-user", "materialize-user"].includes(entry.action) && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
												type: "button",
												style: {
													...S.btn,
													padding: "2px 7px"
												...
												onClick: () => {
													migrateHarnessProvider(entry);
												},
												children: providerLifecycleBusy === `migrate:${entry.provider_id}` ? copy.providerLayerMigrating : copy.providerLayerMigrate
											})
										]
									}, entry.provider_id))
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("a", {
									href: "http://127.0.0.1:3210/",
									target: "_blank",
									rel: "noopener noreferrer",
									style: {
										...S.btn,
									...
									children: [copy.providerLayerMigrationPlan, " → 3210"]
								})
							]
						}),
						...
						(providerInventory?.records ?? []).map((record) => {
							...
							return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									...
									immutable && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										style: { opacity: .65 },
										children: copy.providerOfficialBuiltin
									}),
									unresolved && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
										style: { color: "#c98735" },
										children: copy.providerSourceUnresolved
									}),
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { style: { flex: 1 } }),
									record.delete_capability === "supported" && record.desired_state !== "absent" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										style: {
											...S.btn,
											padding: "2px 8px"
									...
									(record.desired_state === "absent" || [
										...
										"ROLLBACK_RESTORED",
										"ROLLBACK_APPLYING"
									].includes(rollbackTransaction?.state)) && rollbackTransaction && rollbackTransaction.source !== "provider-layer-migration" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										style: {
											...S.btn,
											padding: "2px 8px"
									...
									rollbackTransaction?.source === "provider-layer-migration" && rollbackTransaction.transaction_id && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
										type: "button",
										style: {
											...S.btn,
											padding: "2px 8px"
										...
										disabled: providerLifecycleBusy !== null,
										onClick: () => {
											rollbackHarnessMigration(record, rollbackTransaction.transaction_id);
										},
										children: providerLifecycleBusy === `rollback-migrate:${record.id}` ? copy.working : copy.providerLayerRollback
									})
								]
							}, record.id);
						}),
						(providerInventory?.recovery_transactions ?? []).filter((entry) => entry?.unresolved === true || !entry?.provider_id).map((entry) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							style: {
								display: "flex",
								alignItems: "center",
								gap: 8,
							...
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									style: { fontWeight: 600 },
									children: copy.recoveryUnresolved
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									style: S.mono,
									children: entry.storage_id ?? entry.transaction_id ?? "unknown"
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									style: { opacity: .7 },
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { style: { flex: 1 } }),
								entry.action_id && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									style: {
										...S.btn,
										padding: "2px 8px"
									...
									disabled: providerLifecycleBusy !== null,
									onClick: () => {
										quarantineRecoveryTransaction(entry);
									},
									children: providerLifecycleBusy === `quarantine:${entry.action_id}` ? copy.working : copy.recoveryQuarantine
								})
							]
						}, `recovery:${entry.storage_id ?? entry.transaction_id ?? "unknown"}`)),
						(credentialRefs.length > 0 || credentialUnverified.length > 0) && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							style: {
								marginTop: 10,
							...
							children: [
								...
								credentialUnverified.map((entry) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									style: {
										color: "#c55",
										fontSize: 11.5
									},
									children: [
										/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											style: S.mono,
											children: entry.reference_id
										}),
										" · ",
										copy.credentialPurgeUnverified
									]
								}, `unverified:${entry.reference_id}`)),
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
										...
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
											...
											children: [
												...
												" · ",
												entry.roles.length > 0 ? entry.roles.join(" / ") : copy.never
											]
										})]
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
