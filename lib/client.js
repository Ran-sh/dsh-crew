...
function normalizeSchedule(value) {
	...
	const offset = Number.isInteger(source.timezone_offset_minutes) && Math.abs(source.timezone_offset_minutes) <= 840 ? source.timezone_offset_minutes : SCHEDULE_DEFAULT_OFFSET;
	const weekdays = Array.isArray(source.weekdays) ? [...new Set(source.weekdays.filter((d) => Number.isInteger(d) && d >= 0 && d <= 6))].sort((a, b) => a - b) : [
		1,
		2,
		3,
		4,
	...
	const windows = Array.isArray(source.peak_windows) ? source.peak_windows.filter((w) => scheduleClock(w?.start) !== null && scheduleClock(w?.end) !== null && scheduleClock(w.start) !== scheduleClock(w.end)).map((w) => ({
	...
	return {
		...
		models: (Array.isArray(source.models) ? source.models : []).filter((row) => {
