...
function modelCallability(readinessSnapshot) {
	const rows = Array.isArray(readinessSnapshot?.readiness_matrix?.rows) ? readinessSnapshot.readiness_matrix.rows : [];
	if (rows.find((row) => [
		...
		"deepseek_flash",
		"deepseek_pro"
	].includes(row?.id) && row?.status === "PASS")) return READINESS_STATES.READY;
	if (rows.some((row) => row?.id === "provider_health" && row?.status === "FAIL")) return READINESS_STATES.UNAVAILABLE;
	if (rows.some((row) => ["provider_health", "provider_catalog"].includes(row?.id) && [
		"PASS",
		"NOT_RUN",
		"SKIP"
	].includes(row?.status))) return READINESS_STATES.DEGRADED;
	return READINESS_STATES.UNKNOWN;
}
