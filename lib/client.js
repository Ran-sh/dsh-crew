...
const DEFAULT_PEAK_WEEKDAYS = Object.freeze([
	1,
	2,
	3,
	4,
...
function peakWindows(value, fallback) {
	if (value === void 0 || value === null) return fallback.map((window) => ({ ...window }));
	if (!Array.isArray(value)) return null;
	const windows = [];
...
*/
function normalizeModelSchedule(raw) {
	const source = raw && typeof raw === "object" && !Array.isArray(raw) ? raw : {};
	const models = modelModes(source.models);
	const offset = offsetMinutes(source.timezone_offset_minutes, 480);
	...
	if (offset === null || weekdays === null || windows === null) return {
		timezone_offset_minutes: offset === null ? 480 : offset,
		weekdays: [],
...
function localParts(at, offset) {
	const shifted = new Date(at.getTime() + offset * 6e4);
	return {
...
*/
function isPeakAt(schedule, at = /* @__PURE__ */ new Date()) {
...
function WorkersPanel({ ctx }) {
	...
	*  legacy 3080 supervisor endpoint is retired. */
	...
	const scheduleModelRefs = (() => {
		...
		const add = (ref) => {
			if (ref?.provider && ref?.model) seen.set(`${ref.provider}\0${ref.model}`, {
		...
		for (const role of ["worker", "review"]) {
		...
		for (const provider of modelCatalog?.providers ?? []) for (const model of provider?.models ?? []) add({
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								options: (() => {
									...
									return [...offsets].sort((a, b) => a - b).map((minutes) => ({
										...
										label: `UTC${minutes >= 0 ? "+" : "-"}${Math.floor(Math.abs(minutes) / 60)}` + (Math.abs(minutes) % 60 ? `:${String(Math.abs(minutes) % 60).padStart(2, "0")}` : "")
									}));
								})()
							})]
						}),
