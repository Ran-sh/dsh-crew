...
test('Harness settings exposes and replaces the real agent-default-model authority', () => {
  const inspected = readHarnessDefault(SETTINGS);
  ...
  const replaced = replaceHarnessDefault(SETTINGS, {
    provider: 'deepseek-official', model: 'deepseek-v4-flash', expectedRevision: inspectProviderSettings(SETTINGS).revision,
  });
  assert.equal(replaced.ok, true);
  assert.match(replaced.text, /agent-default-model:\n  provider: deepseek-official\n  model: deepseek-v4-flash/);
  assert.equal(replaceHarnessDefault(SETTINGS, { provider: 'x', model: 'y', expectedRevision: '0'.repeat(64) }).code, 'PROVIDER_SETTINGS_CHANGED');
});
