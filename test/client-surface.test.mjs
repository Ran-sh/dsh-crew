...
test('official bridge evidence selects the 3080 full control plane', () => {
  ...
  assert.deepEqual(surfaceResponsibilities(surface), { fullControlPlane: true, minimalDiagnostics: false });
});

test('native Hub evidence selects the 3210 diagnostics-only surface', () => {
  const surface = classifyCrewSurface({
    bridgeStatus: null,
  ...
  assert.deepEqual(surfaceResponsibilities(surface), { fullControlPlane: false, minimalDiagnostics: true });
});

test('missing or contradictory evidence fails closed to minimal diagnostics', () => {
