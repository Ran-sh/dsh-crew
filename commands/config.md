...
Then show the returned configuration (including `hub_reachable`) as one compact table, plus the effective `flash_state` / `pro_state` and the short `routing_guidance`. Name the fields that changed if anything did. Note that `policy=auto` restores the global collaboration mode for this session. Reply in the language the user is writing in. Do nothing else.
