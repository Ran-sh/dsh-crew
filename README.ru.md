...
<p align="center">
  <sub>npm: <code>@zseven-w/dsh-crew</code> &middot; Текущий релиз плагина: <code>0.1.0-rc.1</code> &middot; Проверено с DSH <code>0.1.0-rc.6</code></sub>
</p>
...
Smoke test проверяет standalone-путь, поэтому требует ключ выше. Примерно через десять секунд должно появиться `smoke test passed — configuration OK`. При ошибке печатается причина; обычно ключ отсутствует или недействителен.
