...
/**
 * Resolve the repository root + base revision for a cwd. Also reports whether
 * the PRIMARY workspace is dirty so callers can warn that uncommitted user
 * changes are intentionally excluded from the detached worker worktree.
 */
export async function inspectRepository({ cwd, git, runner } = {}) {
  ...
  if (!status.ok) return { ok: false, reason: status.reason, error: status.error };
  return {
    ok: true,
...
/**
 * Build the sanitized candidate patch. Sensitive content is excluded at the
 * git-command boundary; truncation runs only after sanitization. The returned
 * completeness flag says whether the patch is sufficient to reconstruct every
 * worker change without the retained worktree.
 */
...
async function safeNewFilePatch(relPath, absPath) {
  try {
    ...
    if (buf.includes(0)) {
      return {
        patch: `[NEW BINARY FILE: ${relPath} (${buf.length} bytes)]\n`,
        complete: false,
        reason: `binary_untracked:${relPath}`,
      };
    }
    ...
    return {
      patch: `[UNTRACKED FILE: ${relPath}]\n`,
      complete: false,
      reason: `untracked_read_failed:${relPath}`,
    };
  }
}
 ...
 * both captured. `complete/replayable=false` means the worktree must be kept
 * because the bounded/redacted candidate is not a full reconstruction.
 */
export async function captureCandidate({ worktreePath, baseRevision, git, limit = DIFF_LIMIT } = {}) {
  ...
  const built = await buildCandidatePatch(run, {
    cwd: worktreePath,
    base,
...
/** Remove an isolated worktree from the MAIN repository. */
...
/** List stale dsh-crew worktrees outside the allowed set. */
