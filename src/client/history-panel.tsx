...
const copy = {
  zh: {
    ...
    operation: '操作', archive: '归档（可恢复）', delete: '删除（不可恢复）', scope: '范围', all: '全部（含你自己的会话）', crew: '仅 Crew 创建', worktree: '仅 Crew 工作区（推荐）', before: '指定时间之前',
  ...
  en: {
    ...
    operation: 'Action', archive: 'Archive (restorable)', delete: 'Delete (permanent)', scope: 'Scope', all: 'Everything (includes your own sessions)', crew: 'Crew-created only', worktree: 'Crew workspaces only (recommended)', before: 'Before a date',
...
export function HistoryPanel({ locale }: { locale: string }) {
  ...
  const [scope, setScope] = useState('worktree');
  ...
  return <details style={{ border: '1px solid rgba(128,128,128,.24)', borderRadius: 10, padding: '10px 13px' }} onToggle={e => setOpen(e.currentTarget.open)}>
    ...
    {open && <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 12 }}>
      ...
      {timeScoped && <label>{t.time} <input type="datetime-local" style={style} value={before} disabled={locked} onChange={e => { setBefore(e.target.value); invalidate(); }} /></label>}
      <div style={{ fontSize: 11.5, opacity: .7 }}>{t.timeHint}</div>
      <button type="button" style={style} disabled={locked || !connected || (timeScoped && before !== '' && !Number.isFinite(new Date(before).getTime())) || (scope === 'before' && before === '')} onClick={() => void act('preview', { operation, scope, ...(timeScoped && before !== '' ? { before: new Date(before).toISOString() } : {}) })}>{t.preview}</button>
